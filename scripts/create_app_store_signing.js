...
const BUNDLE_IDENTIFIER = process.env.BUNDLE_IDENTIFIER || 'com.snarfnet.dubuniverse';
const BUNDLE_NAME = process.env.BUNDLE_NAME || 'DUB UNIVERSE';
const OUTPUT_DIR = process.env.SIGNING_OUTPUT_DIR || `${process.env.USERPROFILE}/DUB_UNIVERSE_SIGNING`;
...
async function main() {
  ...
  console.log(`Bundle ID: ${bundle.id} ${bundle.attributes.identifier}`);

  const { privateKey, privateKeyPem, csrPem } = makeCSR();
  const cert = await createCertificate(csrPem);
  console.log(`Certificate: ${cert.id} ${cert.attributes.certificateType}`);
  const certFiles = writeCertificateFiles(privateKey, privateKeyPem, cert);

  const profile = await createProfile(bundle.id, cert.id);
  console.log(`Profile: ${profile.id} ${profile.attributes.name}`);
  const profileFiles = writeProfileFiles(profile);

  console.log(JSON.stringify({
    outputDir: OUTPUT_DIR,
    p12: certFiles.p12,
    p12Password: certFiles.p12Password,
    provisioningProfile: profileFiles.profile,
    provisioningProfileName: profile.attributes.name,
  }, null, 2));
}
